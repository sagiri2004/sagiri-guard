...
#include <stdlib.h>
#include <string.h>
#include <sys/stat.h> // Dùng cho hàm _stat (tương đương stat trên POSIX)
...
// Tương tự, liên kết với thư viện WinHTTP, dùng cho các hàm HTTPS.
#pragma comment(lib, "winhttp.lib")

#include <windows.h> // Header chính của Windows, chứa winsock2.h
#include <winhttp.h>   // Header cho các hàm HTTPS (WinHTTP)
...
// là các hàm tiện ích (wrapper) gọi đến http_request
// với các tham số tương ứng.

/**
 * @brief Hàm tiện ích cho HTTP GET. (Wrapper cho http_request)
 */
int http_get(const char* host, int port, const char* path,
             const char* extra_headers, char* response, size_t response_len) {
    // GET không có content_type và body (body_len = 0)
    return http_request(host, port, "GET", path, NULL, NULL, 0, extra_headers, response, response_len);
}

/**
 * @brief Hàm tiện ích cho HTTP POST. (Wrapper cho http_request)
 */
...
/**
 * @brief Hàm tiện ích cho HTTP PUT. (Wrapper cho http_request)
 */
...
/**
 * @brief Hàm tiện ích cho HTTP DELETE. (Wrapper cho http_request)
 */
 ...
 * (Hàm này đã bị xóa trong code bạn cung cấp - "http_post_file removed (unused)")
 */
// http_post_file removed (unused)


// --- Các hàm HTTPS (Sử dụng WinHTTP) ---

/**
 * @brief Thực hiện yêu cầu HTTPS GET sử dụng thư viện WinHTTP.
 * @details Không thể dùng socket TCP cơ bản cho HTTPS vì cần xử lý mã hóa SSL/TLS.
 * Hàm này sử dụng API WinHTTP (một thư viện cấp cao của Windows) để:
 * 1. Mở một session WinHTTP (WinHttpOpen).
 * 2. Kết nối đến server (WinHttpConnect).
 * 3. Tạo một request (WinHttpOpenRequest) với cờ WINHTTP_FLAG_SECURE.
 * 4. (Tùy chọn) Thêm các header (WinHttpAddRequestHeaders).
 * 5. Gửi request (WinHttpSendRequest) và nhận phản hồi (WinHttpReceiveResponse).
 * 6. Đọc dữ liệu phản hồi (WinHttpReadData) vào buffer 'response'.
 * 7. Đóng các "handle" (HINTERNET) của WinHTTP.
 * @param host Host của server.
 * @param port Cổng (mặc định 443 nếu <= 0).
 * @param path Đường dẫn (ví dụ: "/api/data").
 * @param extra_headers Các header bổ sung (định dạng "Header: Value\r\n").
 * @param response Buffer (output) để lưu phản hồi (chỉ body).
 * @param response_len Kích thước của buffer 'response'.
 * @return 0 nếu thành công, -1 nếu thất bại.
 */
int https_get(const char* host, int port, const char* path,
    ...
    if (use_default_port) port = 443; // Cổng mặc định cho HTTPS

    // WinHTTP API sử dụng Wide-Char (wchar_t), nên cần chuyển đổi chuỗi
    wchar_t whost[256];
    wchar_t wpath[1024];
    ...
    // 3. Tạo Request
    // WINHTTP_FLAG_SECURE báo cho WinHTTP biết đây là kết nối HTTPS
    ...
    // 5. Gửi Request (GET không có body)
    ...
    // 5.1. Nhận Phản hồi (chờ server xử lý)
    bResults = WinHttpReceiveResponse(hRequest, NULL);
    if (!bResults) { WinHttpCloseHandle(hRequest); WinHttpCloseHandle(hConnect); WinHttpCloseHandle(hSession); return -1; }

    // 6. Đọc Dữ liệu (Body của Response)
    DWORD dwSize = 0, dwDownloaded = 0; size_t total = 0;
    do {
        // Kiểm tra xem có bao nhiêu data đang chờ
        if (!WinHttpQueryDataAvailable(hRequest, &dwSize)) { break; }
        if (dwSize == 0) break; // Không còn data
        // Đảm bảo không đọc quá kích thước buffer (chừa 1 byte cho '\0')
        if (total + dwSize + 1 > response_len) { dwSize = (DWORD)(response_len - total - 1); }
        if (dwSize == 0) break; // Buffer đầy
        // Đọc data vào buffer
        if (!WinHttpReadData(hRequest, response + total, dwSize, &dwDownloaded)) { break; }
        total += dwDownloaded;
    } while (dwSize > 0 && total + 1 < response_len);
    response[total] = '\0'; // Kết thúc chuỗi
...
/**
 * @brief Hàm trợ giúp *nội bộ* cho các request HTTPS có body (POST, PUT, DELETE).
 * @details Tương tự như 'https_get', nhưng hàm này được thiết kế để xử lý
 * các phương thức cần gửi dữ liệu (body) và Content-Type.
 * Nó truyền body và body_len vào hàm 'WinHttpSendRequest'.
 * @param method Phương thức (dưới dạng Wide-Char, ví dụ: L"POST").
 * @param host Host.
 * @param port Cổng.
 * @param path Đường dẫn.
 * @param content_type Loại nội dung (sẽ được thêm vào header).
 * @param body Dữ liệu (body) cần gửi.
 * @param body_len Độ dài của body.
 * @param extra_headers Các header bổ sung.
 * @param response Buffer (output) lưu phản hồi.
 * @param response_len Kích thước buffer 'response'.
 * @return 0 nếu thành công, -1 nếu thất bại.
 */
static int https_body_request(const wchar_t* method, const char* host, int port, const char* path,
    ...
    // Chuyển đổi chuỗi sang Wide-Char
    wchar_t whost[256];
    wchar_t wpath[1024];
    MultiByteToWideChar(CP_UTF8, 0, host, -1, whost, 256);
    MultiByteToWideChar(CP_UTF8, 0, path, -1, wpath, 1024);
    ...
    HINTERNET hConnect = WinHttpConnect(hSession, whost, (INTERNET_PORT)port, 0);
    if (!hConnect) { WinHttpCloseHandle(hSession); return -1; }
    // 3. Tạo Request (với phương thức được truyền vào)
    ...
    // 5. Gửi Request (Khác với GET: truyền body và body_len vào đây)
    ...
    // 6. Đọc Dữ liệu (Giống hệt https_get)
    DWORD dwSize = 0, dwDownloaded = 0; size_t total = 0;
    do {
        if (!WinHttpQueryDataAvailable(hRequest, &dwSize)) { break; }
        if (dwSize == 0) break;
